...
def _parse(rows: list[dict], location: str = "oerlikon") -> list[tuple[datetime, int]]:
    """
    Raw dicts (from asyncpg) → (naive local datetime, occupancy) in Europe/Zurich.

    Each row has a `ts` key (datetime with tzinfo from PostgreSQL) and
    `occupancy_oerlikon` / `occupancy_city` integer columns.
    """
    ...
    for r in rows:
        ...
        if ts.tzinfo is not None:
            ts = ts.astimezone(_TZ)
        out.append((ts.replace(tzinfo=None), int(occ)))
    return out
...
def build_dashboard_html(
    ...
    """Build charts and return a complete HTML page.

    Args:
        readings: List of dicts from db.fetch_readings() — each has
                  ts (datetime), occupancy_oerlikon (int|None), occupancy_city (int|None).
        lookback_days: Used for the subtitle only.
